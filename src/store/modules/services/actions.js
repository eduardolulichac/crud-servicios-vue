...
export default {
  ...
  saveService: ({ getters, dispatch }, payload) => {
    return new Promise((resolve, reject) => {
      firebase
        .database()
        .ref("services")
        ...
        .then(() => {
          if (
            payload.category == getters.filterType ||
            getters.filterType == "Todos"
          ) {
  ...
  updateService: ({ getters, dispatch }, payload) => {
    return new Promise((resolve, reject) => {
      firebase
        .database()
        .ref("services")
        .child(payload.id)
        .update(payload.dataEdit)
        .then(() => {
          dispatch("getListServices", getters.filterType);
          resolve(true);
        })
  ...
  deleteService: ({ getters, dispatch }, payload) => {
    return new Promise((resolve, reject) => {
      firebase
        .database()
        .ref("services")
